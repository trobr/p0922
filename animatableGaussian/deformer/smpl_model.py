...
class SMPLModel(nn.Module):
    ...
    def forward(self, body_pose, global_orient, transl, time, iteration, total_iteration, is_use_ao=False, **kwargs):
        ...
        """
        torch.cuda.nvtx.range_push("s-forward")
        torch.cuda.nvtx.range_push("s-enc")
        ...
        if self.use_deform_network:
            # 准备输入参数
            torch.cuda.nvtx.range_push("s-def-pre")
            ...
            torch.cuda.nvtx.range_push("s-def-forward")
            ...
            torch.cuda.nvtx.range_push("s-def-post")
            # # 变形后的数据统计
        ...
        torch.cuda.nvtx.range_push("s-lbs")
