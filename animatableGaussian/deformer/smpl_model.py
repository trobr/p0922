...
class SMPLModel(nn.Module):
    ...
    def forward(self, body_pose, global_orient, transl, time, iteration, total_iteration, is_use_ao=False, **kwargs):
        ...
        """
        torch.cuda.nvtx.range_push("smpl_forward")
        torch.cuda.nvtx.range_push("smpl_enc")
        ...
        torch.cuda.nvtx.range_pop()  # smpl_enc
        

        # 新增：使用 deform_network 进行变形
        if self.use_deform_network:
            # 准备输入参数
            torch.cuda.nvtx.range_push("smpl_def_pre")
            ...
            torch.cuda.nvtx.range_pop()  # smpl_def_pre
            
            # 调用 deform_network.forward
            torch.cuda.nvtx.range_push("smpl_def_forward")
            ...
            torch.cuda.nvtx.range_pop()  # smpl_def_forward
            
            torch.cuda.nvtx.range_push("smpl_def_post")
            # 变形后的数据统计
            ...
            torch.cuda.nvtx.range_pop()  # smpl_def_post
        else:
        ...
        torch.cuda.nvtx.range_pop()  # smpl_forward
